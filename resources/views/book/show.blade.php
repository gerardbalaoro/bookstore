        ...
        <div class="mx-auto lg:w-4/5 mb-4 px-2">
            <div class="h-full md:p-8 rounded-lg mt-24 lg:mt-auto overflow-visible shadow-lg bg-white flex flex-wrap flex-col lg:flex-row">
                <div class="bg-grey-100 w-full lg:w-1/3 lg:pr-6 magnific" href="{{ $book->cover(600) }}">
                    ...
                    data-src="{{ $book->cover(350) }}" alt="{{ $book->title }} Cover">
                </div>
                    ...
                    <div class="py-3 flex flex-col">
                        <div class="flex flex-wrap justify-center mb-3">
                            @foreach ($book->authors as $author)
                                @component('components.icon-card', ['subtitle' => 'Author', 'url' => $author->url])
                                    @slot('icon')
                                ...
                                @endcomponent                               
                            @endforeach
                            @component('components.icon-card', ['subtitle' => 'Publication Date', 'class' => 'cursor-default'])
                                    @slot('icon')
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                            ...
                            @if ($book->publishers->count())
                                @component('components.icon-card', ['subtitle' => 'Publisher', 'url' => $book->publishers[0]->url])
                                    @slot('icon')
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                            ...
                            @foreach ($book->identifiers->where('type', '=', 'isbn') as $isbn)
                                @component('components.icon-card', ['subtitle' => 'ISBN', 'url' => "https://worldcat.org/isbn/{$isbn->val}", 'external' => true])
                                    @slot('icon')
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                        <path d="M13.6 13.47A4.99 4.99 0 0 1 5 10a5 5 0 0 1 8-4V5h2v6.5a1.5 1.5 0 0 0 3 0V10a8 8 0 1 0-4.42 7.16l.9 1.79A10 10 0 1 1 20 10h-.18.17v1.5a3.5 3.5 0 0 1-6.4 1.97zM10 13a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"></path>
                                    </svg>
                                    @endslot
                                    {{ $isbn->val }}
                                @endcomponent
                        ...
                        <div class="px-6 py-2 flex flex-wrap justify-center">
                            @foreach ($book->identifiers->where('type', '!=', 'isbn')->where('linkable') as $id)
